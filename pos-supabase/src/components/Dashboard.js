...
const Dashboard = ({ auth, setAuth }) => {
    ...
    try {
      ...
      const topProductsByValue = products
        .map((p) => ({
          ...
          potential_profit:
            (p.stock_quantity || 0) *
            ((p.price || 0) - (p.cost_price || p.price * 0.7)),
        }))
  ...
  const fetchSalesReport = async (start, end) => {
    ...
    const formattedOrders = orders.map((order) => ({
      ...
      profit_amount:
        parseFloat(order.profit_amount) || order.total_amount * 0.3,
    }));
    ...
    const totalProfit = formattedOrders.reduce(
      (sum, o) => sum + o.profit_amount,
      0,
    );
    const avgOrderValue =
      formattedOrders.length > 0 ? totalSales / formattedOrders.length : 0;
  ...
  const fetchProfitReport = async (start, end) => {
    ...
    const profitData = Object.values(dailyProfit).sort(
    ...
    setReportSummary({
      ...
      totalProfit,
      avgDailyProfit:
        profitData.length > 0 ? totalProfit / profitData.length : 0,
  ...
  const fetchStaffReport = async (start, end) => {
    ...
    const staffData = Array.from(staffMap.values()).sort(
      (a, b) => b.revenue - a.revenue,
    );
  ...
  const exportToCSV = () => {
    ...
    const headers = Object.keys(reportData[0]).filter(
      (key) => !["id", "users", "products", "categories"].includes(key),
    );
        ...
        <div className="actions-grid">
          ...
          {auth.user?.role === "admin" && (
            <button
              ...
              style={{
                background: showReports ? "#4299e1" : "#f7fafc",
                cursor: "pointer",
                border: "none",
                width: "100%",
                textAlign: "left",
          ...
          {reportData.length > 0 && (
            <div className="report-footer">
              <p>
                ...
                {new Date(getDateRangeForReport().start).toLocaleDateString()} -{" "}
                {new Date(getDateRangeForReport().end).toLocaleDateString()}
              </p>
              <p>
                  ...
                  topProducts.map((product, index) => (
                    <div key={product.id} className="product-item">
                      ...
                      <div className="product-meta">
                        ...
                        <div>
                          Value:{" "}
                          <strong>{formatCurrency(product.stock_value)}</strong>
                        </div>
                        <div className="potential-profit">
                          Potential Profit:{" "}
                          {formatCurrency(product.potential_profit)}
                        </div>
                      </div>
                    </div>
                  ))
